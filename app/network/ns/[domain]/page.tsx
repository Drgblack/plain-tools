...
export function generateStaticParams() {
  return generateNetworkNsParams(getPrebuildLimit())
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!normalizedDomain || !isValidDnsDomain(normalizedDomain)) {
    const invalid = buildPageMetadata({
      description: "The requested domain is not valid for NS lookup.",
      googleNotranslate: true,
      image: "/og/default.png",
      path: `/network/ns/${encodeURIComponent(domain)}`,
      title: "Invalid NS lookup domain | Plain Tools",
    })
    return { ...invalid, robots: { follow: false, index: false } }
  }

  const page = buildNsPage(normalizedDomain, [])
...
function ResultsSection({
  ...
  answers: Awaited<ReturnType<typeof fetchNsLookup>>["answers"]
  domain: string
}) {
  return (
    <section className="space-y-6 notranslate" translate="no">
        ...
        <div className="mt-4">
          <NetworkStatGrid
            items={[
              ...
              { label: "Lookup type", value: "NS" },
              { label: "Focus", value: "Delegation" },
      ...
      <section className="rounded-2xl border border-border/80 bg-card/60 p-5 shadow-[0_12px_40px_-28px_rgba(0,112,243,0.35)]">
        <NetworkListTable
          columns={["Host", "Nameserver", "TTL"]}
          rows={
            answers.length > 0
              ? answers.map((answer) => [answer.name, answer.data, `${answer.ttl}s`])
              : [[domain, "No NS records returned", "-"]]
          }
        />
      </section>
    </section>
...
export default async function NetworkNsPage({ params }: Props) {
  const { domain } = await params
  const normalizedDomain = normalizeDnsDomain(domain)
  ...
  if (domain !== normalizedDomain) {
    permanentRedirect(`/network/ns/${encodeURIComponent(normalizedDomain)}`)
  }

  const lookup = await fetchNsLookup(normalizedDomain).catch(() => ({
    answers: [],
    resolver: "google" as const,
    status: -1,
  }))
  const page = buildNsPage(normalizedDomain, lookup.answers)
  ...
  return (
    <ProgrammaticLayout
      ...
      liveTool={<NetworkLookupForm defaultValue={normalizedDomain} kind="ns" />}
      liveToolDescription={page.liveToolDescription}
      liveToolTitle="Run another NS lookup"
      page={page.page}
      relatedSectionTitle={`Related checks after NS lookup for ${normalizedDomain}`}
